...
| 12  | File watcher                   | `chokidar`, 200ms debounce, ships in v0.1                                                                                                                                |
...
| 15  | Vector store                   | `sqlite-vec@0.1.9` + `better-sqlite3@^11`, single `.sqlite` file                                                                                                         |
| 16  | Graph library                  | `graphology@^0.26` + standard plugins                                                                                                                                    |
...
| 20  | inputSchema generation         | Hand-written JSON Schemas per tool (NOT `zod-to-json-schema(schema, name)` which emits `$ref/definitions` unacceptable to some MCP clients)                              |
...
| 22  | Error handling in server       | All tool invocations wrapped; thrown errors converted to `McpError` with proper JSON-RPC codes                                                                           |
...
- 2026-04-27 Wave 1: scaffold + CI workflows + fixture generator landed; `npm install/build/typecheck/lint/format:check/test/quality` all green locally; `npm run gen:vault --size 10` produces the 11-file canonical fixture; git repo initialized with remote `origin` (not pushed); `grype` surfaces transitive Go-stdlib findings so it is left for CI (per spec); remaining Wave 1 item (`gh workflow list` + initial CI run green) is deferred to the user's first push; next action: Wave 2 (parsers + resolver + keyword tools).
